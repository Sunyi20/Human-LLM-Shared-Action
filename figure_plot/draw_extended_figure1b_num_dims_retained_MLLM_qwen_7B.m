...
load(fullfile('data/MLLM_qwen_7B/qwen_spose_similarity.mat'))
% spose_sim=embedding2sim(spose_embedding31);
dissim = 1-similarity_matrix;
% load test set
spose_embedding31= load(fullfile('data/MLLM_qwen_7B/qwen_7B_VL_spose_embedding_sorted_final.txt'));
% get dot product (i.e. proximity)
dot_product31= spose_embedding31*spose_embedding31';

% load 10% validation (i.e., test) data
triplet_testdata31 = load(fullfile('data/MLLM_qwen_7B/test_triplets_mllm.txt'))+1; % 0 index -> 1 index
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
behav_predict = zeros(length(triplet_testdata31),1);
behav_predict_prob = zeros(length(triplet_testdata31),1);
rng(42) % for reproducibility
for i = 1:length(triplet_testdata31)
    sim(1) = dot_product31(triplet_testdata31(i,1),triplet_testdata31(i,2));
    sim(2) = dot_product31(triplet_testdata31(i,1),triplet_testdata31(i,3));
    sim(3) = dot_product31(triplet_testdata31(i,2),triplet_testdata31(i,3));
...
behav_predict_acc = 100*mean(behav_predict==3);
% get prediction for each object
for i_obj = 1:355
    behav_predict_obj(i_obj,1) = 100*mean(behav_predict(any(triplet_testdata31==i_obj,2))==3);
    ...
    behav_predict_obj_prob(i_obj,1) = 100*mean(behav_predict_prob(any(triplet_testdata31==i_obj,2)));
end
...
fn1 = fullfile('data/MLLM_qwen_7B/qwen_spose_similarity_reduced.mat');
fn2 = fullfile('data/MLLM_qwen_7B/qwen_spose_embedding31_reduced.mat');

if ~exist(fn1,'file') || ~exist(fn2','file')
    [~,embedding_sortind] = sort(spose_embedding31,2);
    ...
    for i_dim = 1:31
        fprintf('.')
        % make 31 reduced versions, make 31 reduced similarity matrices
        if i_dim == 1
            qwen_spose_embedding31_reduced{i_dim,1} = spose_embedding31;
        else
            qwen_spose_embedding31_reduced{i_dim,1} = qwen_spose_embedding31_reduced{i_dim-1,1};
        end
        for i = 1:355
            qwen_spose_embedding31_reduced{i_dim,1}(i,embedding_sortind(i,i_dim)) = 0;
        end
        qwen_spose_similarity_reduced{i_dim,1} = embedding2sim(qwen_spose_embedding31_reduced{i_dim,1});
        
    end
    fprintf('\n')
    save(fn1,'data/MLLM_qwen_7B/qwen_spose_similarity_reduced')
    save(fn2,'data/MLLM_qwen_7B/qwen_spose_embedding31_reduced')
else
    load(fn1)
    load(fn2)
end
...
for i_dim = 1:31
    rng(42) % for reproducibility
    behav_predict = zeros(length(triplet_testdata31),1);
    dot_product31_reduc = qwen_spose_embedding31_reduced{i_dim}*qwen_spose_embedding31_reduced{i_dim}';
    for i = 1:length(triplet_testdata31)
        sim(1) = dot_product31_reduc(triplet_testdata31(i,1),triplet_testdata31(i,2));
        sim(2) = dot_product31_reduc(triplet_testdata31(i,1),triplet_testdata31(i,3));
        sim(3) = dot_product31_reduc(triplet_testdata31(i,2),triplet_testdata31(i,3));
        [m,mi] = max(sim);
        if sum(sim==m)>1, tmp = find(sim==m); mi = tmp(randi(sum(sim==m))); m = sim(mi); end % break ties choosing randomly (reproducible by use of rng)
        behav_predict(i,1) = mi;
    end
    % get overall prediction (predict choice == 1)
    behav_predict_acc_reduc(i_dim) = 100*mean(behav_predict==3);
    % get prediction for each object
    for i_obj = 1:355
        behav_predict_obj_reduc(i_obj,i_dim) = 100*mean(behav_predict(any(triplet_testdata31==i_obj,2))==3);
    end
...
for i_dim = 1:31
    r_reduc(i_dim) = corr(squareformq(similarity_matrix),squareformq(qwen_spose_similarity_reduced{i_dim}));
end
...
hcb.EdgeColor = 'none';
% hcb.FaceAlpha = 0.5;
plot(0:31,[behav_predict_acc_reduc behav_predict_acc],'k','LineWidth',3)
plot([0 31],[100/3 100/3],'--r')
...
title('Prediction of MLLM behavior', 'FontSize', 20)

ax = gca;
set(ax, 'FontSize', 17);

if dosave
    print(fig, 'acc_vs_dims_retained_MLLM_qwen_7B', '-dpdf', '-bestfit');
end
