...
load(fullfile('data/MLLM_qwen_72B/qwen_72B_VL_spose_similarity.mat'))
...
triplet_testdata30 = load(fullfile('data/MLLM_qwen_72B/test_triplets_mllm.txt'))+1; % 0 index -> 1 index
...
fn1 = fullfile('data/MLLM_qwen_72B/qwen_spose_similarity_reduced.mat');
fn2 = fullfile('data/MLLM_qwen_72B/qwen_spose_embedding_reduced.mat');
...
if ~exist(fn1,'file') || ~exist(fn2','file')
    ...
    for i_dim = 1:30
        ...
        if i_dim == 1
            qwen_spose_embedding30_reduced{i_dim,1} = spose_embedding30;
        else
            qwen_spose_embedding30_reduced{i_dim,1} = qwen_spose_embedding30_reduced{i_dim-1,1};
        end
        for i = 1:355
            qwen_spose_embedding30_reduced{i_dim,1}(i,embedding_sortind(i,i_dim)) = 0;
        end
        qwen_spose_similarity_reduced{i_dim,1} = embedding2sim(qwen_spose_embedding30_reduced{i_dim,1});
        
    end
    fprintf('\n')
    save(fn1,'qwen_spose_similarity_reduced')
    save(fn2,'qwen_spose_embedding30_reduced')
else
    load(fn1)
    load(fn2)
end
...
for i_dim = 1:30
    rng(42) % for reproducibility
    behav_predict = zeros(length(triplet_testdata30),1);
    dot_product30_reduc = qwen_spose_embedding30_reduced{i_dim}*qwen_spose_embedding30_reduced{i_dim}';
    for i = 1:length(triplet_testdata30)
...
if dosave
    print(fig, 'acc_vs_dims_retained_MLLM_qwen_72B', '-dpdf', '-bestfit');
end
