...
load(fullfile('data/Human/human_spose_similarity.mat'))
dissim = 1-similarity_matrix;
% load 10% validation (i.e., test) data
triplet_testdata30 = load(fullfile('data/Human/test_triplets_human.txt'))+1; % 0 index -> 1 index
...
behav_predict_acc_ci95 = 1.96*std(behav_predict_obj)/sqrt(768);
...
fn1 = fullfile('data/Human/human_spose_similarity_reduced.mat');
fn2 = fullfile('data/Human/human_spose_embedding30_reduced.mat');
...
if ~exist(fn1,'file') || ~exist(fn2','file')
    ...
    for i_dim = 1:30
        ...
        if i_dim == 1
            human_spose_embedding30_reduced{i_dim,1} = spose_embedding30;
        else
            human_spose_embedding30_reduced{i_dim,1} = human_spose_embedding30_reduced{i_dim-1,1};
        end
        for i = 1:768
            human_spose_embedding30_reduced{i_dim,1}(i,embedding_sortind(i,i_dim)) = 0;
        end
        human_spose_similarity_reduced{i_dim,1} = embedding2sim(human_spose_embedding30_reduced{i_dim,1});
        
    end
    fprintf('\n')
    save(fn1,'data/Human/human_spose_similarity_reduced')
    save(fn2,'data/Human/human_spose_embedding30_reduced')
else
    load(fn1)
    load(fn2)
end
...
for i_dim = 1:30
    rng(42) % for reproducibility
    behav_predict = zeros(length(triplet_testdata30),1);
    dot_product30_reduc = human_spose_embedding30_reduced{i_dim}*human_spose_embedding30_reduced{i_dim}';
    for i = 1:length(triplet_testdata30)
    ...
    behav_predict_acc_reduc_ci95(i_dim,1) = 1.96* std(behav_predict_obj_reduc(:,i_dim))/sqrt(768);
end
...
for i_dim = 1:30
    r_reduc(i_dim) = corr(squareformq(similarity_matrix),squareformq(human_spose_similarity_reduced{i_dim}));
end
...
title('Prediction of Human behavior', 'FontSize', 20)
