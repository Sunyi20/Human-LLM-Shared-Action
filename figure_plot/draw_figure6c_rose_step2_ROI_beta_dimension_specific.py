...
def roi_mask(subject, roi_name, roi_all_names, roi_index):
    roi_verts_dict = cortex.utils.get_roi_verts(subject)
            
    if roi_name in roi_verts_dict:
        verts_indices = roi_verts_dict[roi_name]
        mask = np.zeros(roi_index.shape[1], dtype=bool)
        valid_indices = [v for v in verts_indices if v < roi_index.shape[1]]
        mask[valid_indices] = True
        return mask
    else:
        print(f"Warning: ROI '{roi_name}' not found inside pycortex subject '{subject}'")
        return np.zeros(roi_index.shape[1], dtype=bool)
...
def main(args):
    ...
    custom_rois = ['section_1', 'section_2', 'section_3']
    ...
    if results:
        ...
        if avg_roibetas:
            output_filename_npy = pjoin(args.outdir, f'{args.model}_specific_roi_betas.npy')
