...
def draw_rose_diagram(
    ...
    for dim_index in top_indices:
        ...
        fontsize_by_length = radial_room_inches * 72 / max(len(label) * 0.4, 1)
        fontsize_by_width = tangential_room_inches * 100
        fontsize = max(4, min(12, fontsize_by_length, fontsize_by_width))
...
def extract_specific_frames(gif_path, frame_indices, target_size=(512, 512)):
    frames = []
    try:
        with Image.open(gif_path) as gif:
            ...
            for idx in frame_indices:
                if idx < 1 or idx > total_frames:
                    print(f"警告: 帧索引 {idx} 超出范围 (1-{total_frames}) for {gif_path}。跳过此帧。")
                    continue
                gif.seek(idx - 1)
                frame = np.array(gif.convert('RGB').resize(target_size))
                frames.append(frame)
    except Exception as e:
        print(f"打开或读取GIF时出错 {gif_path}: {e}")
    return frames
...
def create_filmstrip_image(frames_list, gap=3):
    if not frames_list:
        return None
...
def process_gif_to_image_array(input_gif_path, frame_indices):
    try:
        ...
        print(f"处理 {input_gif_path} 时出错: {str(e)}")
    return None
