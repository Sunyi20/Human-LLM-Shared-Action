...
% load embedding
spose_embedding = load('data/MLLM_qwen_72B/qwen_72B_VL_spose_embedding_sorted_final.txt');
dim = size(spose_embedding,2);
dosave = 1;

refdir = fullfile('data/MLLM_qwen_72B/reference_models_qwen_spose_threshold_0.4');
for i_model = 1:64
...
reproducibility_ci95_z = 1.96 * std(atanh(reproducibility_clipped), [], 2) / sqrt(64);
...
xlabel({'MLLM embedding dimension index'}, 'FontSize', 20);
hax = gca;
hax.LineWidth = 1.5; 
if dosave
    print(fig, 'dim_reproducibility_MLLM_qwen_72B', '-dpdf', '-bestfit');
end
