...
load(fullfile('data/LLM_deepseek/deepseek_spose_similarity.mat'))
...
triplet_testdata30 = load(fullfile('data/LLM_deepseek/test_triplets_llm.txt'))+1; % 0 index -> 1 index
...
for i_obj = 1:355
    behav_predict_obj(i_obj,1) = 100*mean(behav_predict(any(triplet_testdata30==i_obj,2))==3);
...
fn1 = fullfile('data/LLM_deepseek/deepseek_spose_similarity_reduced.mat');
fn2 = fullfile('data/LLM_deepseek/deepseek_spose_embedding30_reduced.mat');
...
if ~exist(fn1,'file') || ~exist(fn2','file')
    ...
    for i_dim = 1:30
        ...
        if i_dim == 1
            deepseek_spose_embedding30_reduced{i_dim,1} = spose_embedding30;
        else
            deepseek_spose_embedding30_reduced{i_dim,1} = deepseek_spose_embedding30_reduced{i_dim-1,1};
        end
        for i = 1:355
            deepseek_spose_embedding30_reduced{i_dim,1}(i,embedding_sortind(i,i_dim)) = 0;
        end
        deepseek_spose_similarity_reduced{i_dim,1} = embedding2sim(deepseek_spose_embedding30_reduced{i_dim,1});
        
    end
    fprintf('\n')
    save(fn1,'data/LLM_deepseek/deepseek_spose_similarity_reduced')
    save(fn2,'data/LLM_deepseek/deepseek_spose_embedding30_reduced')
else
    load(fn1)
    load(fn2)
end
...
for i_dim = 1:30
    rng(42) % for reproducibility
    behav_predict = zeros(length(triplet_testdata30),1);
    dot_product30_reduc = deepseek_spose_embedding30_reduced{i_dim}*deepseek_spose_embedding30_reduced{i_dim}';
    for i = 1:length(triplet_testdata30)
    ...
    % get prediction for each object
    for i_obj = 1:355
        behav_predict_obj_reduc(i_obj,i_dim) = 100*mean(behav_predict(any(triplet_testdata30==i_obj,2))==3);
    end
...
for i_dim = 1:30
    r_reduc(i_dim) = corr(squareformq(similarity_matrix),squareformq(deepseek_spose_similarity_reduced{i_dim}));
end
...
if dosave
    print(fig, 'acc_vs_dims_retained_LLM_deepseek', '-dpdf', '-bestfit');
end
