...
% load embedding
spose_embedding = load(fullfile('data/LLM_deepseek/deepseek_spose_embedding_sorted_full_sample.txt'));

% spose_embedding = spose_embedding_full(action_indices, :);

dot_product = spose_embedding*spose_embedding';

% load RDM
load(fullfile('data/LLM_deepseek/RDM_48_LLM_deepseek.mat'));
RDM41_triplet = RDM48_triplet(action_indices, action_indices);
...
annotation('textbox', [0, 0.88, 1, 0.1], ...
           'String', 'Deepseek-R1: Comparison of Predicted and Measured RSMs', ...
           'EdgeColor', 'none', ...
           'HorizontalAlignment', 'center', ...
           'FontSize', 22, ...
           'FontWeight', 'bold');
...
if ~exist('dosave', 'var') || dosave
    exportgraphics(fig, 'figures/RSM_Deepseek_R1_41.pdf', ...
                   'ContentType', 'vector', ...
                   'Resolution', 400);
