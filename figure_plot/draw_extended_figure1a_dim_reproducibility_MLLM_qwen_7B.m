...
% load embedding
spose_embedding = load('data/MLLM_qwen_7B/qwen_7B_VL_spose_embedding_sorted_final.txt');
dim = size(spose_embedding,2);
dosave = 1;

refdir = fullfile('data/MLLM_qwen_7B/reference_models_qwen_spose_0.0072');
for i_model = 1:65
...
plot(reproducibility,'o','MarkerFaceColor',[.9 .6 .1],'MarkerEdgeColor','none','MarkerSize',3)
ylim([0 1])
xlim([0 dim+1])
set(gca,'FontSize',17) 
...
hax.LineWidth = 1.5; 
if dosave
    print(fig, 'dim_reproducibility_MLLM_qwen_7B', '-dpdf', '-bestfit');
end
