...
def parseargs():
    ...
    aa('--num_threads', type=int, default=100, help='number of threads used by PyTorch multiprocessing')
...
def run(
    ...
    elif results_dir == './results_update/':
        results_dir = os.path.join(results_dir, modality, f'{embed_dim}d', str(lmbda), f'seed{rnd_seed:02d}')
    else:
        results_dir = os.path.join(results_dir, modality, f'{embed_dim}d', str(lmbda), f'seed{rnd_seed:02d}')
    ...
    # if plots_dir == './plots_update/':
    #     plots_dir = os.path.join(plots_dir, modality, f'{embed_dim}d', str(lmbda), f'seed{rnd_seed}')
    # if plots_dir == './plots_extrapolation/':
    ...
    results = {'epoch': len(train_accs), 'dimensions': current_d, 'train_acc': train_accs[-1], 'val_acc': val_accs[-1], 'val_loss': val_losses[-1]}
