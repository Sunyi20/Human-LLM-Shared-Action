...
triplet_file_path = project_path(
    "list", "noise_ceiling", f"noise_ceiling_set_triplets_segment_{number}.txt"
)
transciptions_folder = project_path("data", "transcriptions_selected")
base_result_dir = project_path(
    "run_experiment",
    "results_qwen2.5_7B_noise_ceiling",
    f"results_qwen2.5_7B_noise_ceiling_triplets_split_{number}",
)
...
csv_save_name = os.path.join(base_result_dir, f'train_chains&responses_qwen2.5_7B_split_{number}_file.csv')
...
def write_trial_data_to_csv(filename, prompts, triplets, responses):
...
def save_files(sorted_images, prompt, response, csv_name, txt_name, select_choice):
    ...
    write_trial_data_to_csv(csv_name, group_prompts, group_results, group_responses)
